...
KGRID  <- 60L
## ONLY the seven cells missing from the v2 run (h = .02 and .04 at the higher
## ratios). Same generator, same engines, same seeds-by-construction.
GRID <- rbind(
  CJ(p_plus = c(.50, .60, .74),      h = .02, K = KGRID),
  CJ(p_plus = c(.41, .50, .60, .74), h = .04, K = KGRID))

OUT <- "/home/claude/outputs/bench_panelA_fill.csv"
